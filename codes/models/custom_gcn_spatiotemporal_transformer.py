...
class SpatioTemporalTransformer(nn.Module):
    def __init__(self, in_channels, gcn_hidden, trans_hidden=64, nhead=4, num_layers=1):
        super().__init__()
        self.gcn = GCNConv(in_channels, gcn_hidden)

        self.proj = nn.Linear(gcn_hidden, trans_hidden)
    ...
    def forward(self, x_seq, edge_index, edge_weight=None):
        ...
        for t in range(T):
            ...
            for b in range(B):
                ...
                x_b = self.gcn(x_b, edge_index, edge_weight)  # [N, H]
                x_t_list.append(x_b)
...
class SpatioTemporalFusionNet(nn.Module):
    def __init__(self, in_channels, graph_feat_dim, trans_hidden=64, out_channels=1, num_nodes=None):
        ...
        self.temporal_encoder = SpatioTemporalTransformer(in_channels, gcn_hidden=32, trans_hidden=trans_hidden)
