...
class GCN_GAT_SpatialEncoder(nn.Module):
    def __init__(self, in_channels, hidden_channels, gat_heads=16):
        super().__init__()

        # GCN layer to process static graph structure & edge weights
        self.gcn = GCNConv(in_channels, hidden_channels)

        # GAT layer to refine with attention across neighbors
        self.gat = GATConv(hidden_channels, hidden_channels, heads=gat_heads, concat=False)

        self.relu = nn.ReLU()

    def forward(self, x, edge_index, edge_weight=None):
        # GCN layer (edge-weight aware)
        x = self.relu(self.gcn(x, edge_index, edge_weight))

        # GAT layer (learned attention, ignores edge weights)
...
class SpatiotemporalTransformer(nn.Module):
    def __init__(self, input_dim, gcn_dim, hidden_dim, nhead, num_layers, num_nodes, forecast_dim=1):
        ...
        if self.use_gcn:
            self.spatial_encoder = GCN_GAT_SpatialEncoder(gcn_dim, hidden_dim, gat_heads=2)
        else:
